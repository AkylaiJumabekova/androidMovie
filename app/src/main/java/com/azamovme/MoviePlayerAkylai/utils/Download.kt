...
object Download {
    ...
    private fun getDownloadDir(activity: Activity): File {
        ...
        if (loadData<Boolean>("sd_dl") == true) {
            ...
            direct = File("storage/emulated/0/${Environment.DIRECTORY_DOWNLOADS}/MoviePlayerAkylai/")
            if (!direct.exists()) direct.mkdirs()
        }
        return direct
    }
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                if (loadData<Boolean>("sd_dl") == true && externalDirs.size > 1 && externalDirs[1] != null) {
                    val parentDirectory =
                        externalDirs[1].toString() + "/MoviePlayerAkylai/${sanitizedTitle}&&${epTitle}/"
                    val directory = File(parentDirectory)
                    if (!directory.exists()) directory.mkdirs()
                    request.setDestinationUri(Uri.fromFile(File("$parentDirectory$sanitizedTitle")))
                } else {
                    val directory =
                        File(Environment.DIRECTORY_DOWNLOADS + "/MoviePlayerAkylai/${sanitizedTitle}&&${epTitle}/")
                    if (!directory.exists()) directory.mkdirs()
                    request.setDestinationInExternalPublicDir(
                        Environment.DIRECTORY_DOWNLOADS,
                        "/MoviePlayerAkylai/${sanitizedTitle}/$epTitle$fileExtension"
                    )
                }
