...
import com.azamovme.MoviePlayerAkylai.repository.TvRepository
import com.azamovme.MoviePlayerAkylai.utils.Color
import com.azamovme.MoviePlayerAkylai.utils.Utils.getJsoup
import com.azamovme.MoviePlayerAkylai.utils.printlnColored
...
class TvRepositoryImpl : TvRepository {
    ...
    override fun getTv() = flow<Result<ArrayList<Movie>>> {
        ...
        if (movieElements != null) {
            ...
            printlnColored("Uzbek channels not found.", com.azamovme.MoviePlayerAkylai.utils.Color.YELLOW)
        }
        movieElements.map {
            val href = it.select("a[href]").attr("href")
            val title = it.select("a[href]").text()
